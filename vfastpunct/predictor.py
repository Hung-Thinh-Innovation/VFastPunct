...
from vfastpunct.constants import MODEL_MAPPING, PUNC_MAPPING, CAP_MAPPING
...
class VFastPunct(object):
    def __init__(self, model_name, no_cuda=False):
        ...
        model_clss,  config_clss, lm_name, model_path, drive_id = MODEL_MAPPING[model_name]
        if not os.path.exists(model_path):
            download_file_from_google_drive(drive_id, model_path, confirm='t')
        self.model, self.tokenizer, self.max_seq_len, self.punc2id =self.load_model(model_clss,  config_clss, lm_name, model_path, self.device)
        self.id2puc = {idx: label for idx, label in enumerate(self.punc2id)}
        self.model.to(self.device)
        self.model.eval()

    @staticmethod
    def load_model(model_clss, config_clss, lm_name, model_path, device='cpu'):
        if device == 'cpu':
        ...
        config = config_clss.from_pretrained(lm_name,
...
class VFastPunctCap(object):
    def __init__(self, model_name, no_cuda=False):
        self.device = 'cuda' if not no_cuda and torch.cuda.is_available() else 'cpu'
        model_clss, config_clss, lm_name, model_path, drive_id = MODEL_MAPPING[model_name]
        # if not os.path.exists(model_path):
        #     download_file_from_google_drive(drive_id, model_path, confirm='t')
        self.model, self.tokenizer, self.max_seq_len, self.punc2id, self.cap2id =self.load_model(model_clss, config_clss, lm_name, model_path, self.device)
        self.id2puc = {idx: label for idx, label in enumerate(self.punc2id)}
    ...
    def load_model(model_clss, config_clss, lm_name, model_path, device='cpu'):
        ...
        tokenizer = AutoTokenizer.from_pretrained(lm_name)
        config = config_clss.from_pretrained(lm_name,
                                             num_plabels=len(checkpoint_data['pclasses']),
                                             num_clabels=len(checkpoint_data['cclasses']))
        model = model_clss(config=config)
        model.load_state_dict(checkpoint_data['model'])
        return model, tokenizer, max_seq_len, checkpoint_data['pclasses'], checkpoint_data['cclasses']
    ...
    def preprocess(self, in_raw: str):
        ...
        num_token = len(tokens)
        while num_token > idx >= 0:
            sents.append(' '.join(tokens[idx: min(idx + self.max_seq_len//2, num_token)]))
            idx += self.max_seq_len//2
        return sents
    ...
    def __call__(self, in_raw: str):
        ...
        for sent in sents:
            item = self.convert_tensor(sent)
            with torch.no_grad():
                ptag, ctag  = self.model(**item)
            for w, plabel, clabel in list(zip(sent.split(), list(itertools.chain(*ptag)), list(itertools.chain(*ctag)))):
                p = PUNC_MAPPING[self.id2puc[plabel]]
                c = self.id2cap[clabel]
                if len(result) > 0 and result[-1] == "." and c != "U":
                    result += f" {w.title()}{p}"
                else:
...
if __name__ == "__main__":
    ...
    import re
    punct = VFastPunctCap("mBertLstmCrf_PuncCap", True)
    while True:
