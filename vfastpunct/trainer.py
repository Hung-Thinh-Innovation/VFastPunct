...
def validate(args,
    ...
    if is_test:
        ...
        LOGGER.info(f"\tValidation Loss: {epoch_loss:.4f} (pLoss: {epoch_ploss:.4f}; cLoss: {epoch_closs:.4f});\n"
                    f"\tAccuracy: {epoch_avg_acc:.4f} (pAccuracy: {preports['accuracy']:.4f}; "
                    f"cAccuracy: {creports['accuracy']:.4f});\n"
                    f"\tMacro-F1 score: {epoch_avg_f1:.4f} (pF1: {preports['macro avg']['f1-score']:.4f}; "
                    f"cF1: {creports['macro avg']['f1-score']:.4f});\n"
...
def test():
    ...
    model_archs = MODEL_MAPPING[configs.task][configs.model_arch]
